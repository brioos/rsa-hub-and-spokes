...
for pkl_file in pkl_files:
    ...
    if roi_name in ling:
        ...
        roi_type = "szenzomotoros-vizuális"
...
sem_corr = df_results.groupby(["Type", "ROI", "Condition"])["Correlation"].agg(
    Correlation="mean",
    SEM=lambda x: np.std(x, ddof=1) / np.sqrt(len(x))
).reset_index()

# adding significance stars
df_ttest_cond["sig_star"] = df_ttest_cond["p-corrected"].apply(
    lambda p: "***" if p < 0.001 else "**" if p < 0.01 else "*" if p < 0.05 else ""
)

merged_data = sem_corr.merge(
    df_ttest_cond[["ROI", "Condition", "sig_star"]],
    on=["ROI", "Condition"],
    how="left"
)
...
for type_ in types:
    type_data = merged_data[merged_data["Type"] == type_]

    with plt.style.context('high-contrast'):
        fig, ax = plt.subplots(figsize=(10, 5))
        sns.barplot(
            data=type_data,
            x="ROI", y="Correlation", hue="Condition",ci=None, ax = ax)

        for i, row in type_data.iterrows():
            xpos = list(type_data["ROI"].unique()).index(row["ROI"])
            cond_offset = -0.2 if row["Condition"] == "abstract" else 0.2
            x = xpos + cond_offset
            ...
            if row["sig_star"]:
                ax.text(x, row["Correlation"] + row["SEM"] + 0.02, row["sig_star"],
                        ha='center', va='bottom', color='red', fontsize=14)

        plt.title(f"Átlagos korreláció – {type_} típusú ROI-k")
        plt.xticks(rotation=45, ha='right')
        plt.ylim(0, type_data["Correlation"].max() + type_data["SEM"].max() + 0.1)
        plt.xlabel("ROI")
        plt.ylabel("Átlagos korreláció")
        plt.legend(title="Feltétel")
        plt.tight_layout()
        plt.tight_layout()
        file_path = os.path.join(figures, f"{type_.replace(' ', '_')}_tbarplot.png")
...
output_path = "../correlations/mean_sd_results.xlsx"

mean_sd_table = df_results.groupby(["ROI", "Condition"]).agg(
    mean_corr=("Correlation", "mean"),
    sd_corr=("Correlation", "std")).reset_index()

with pd.ExcelWriter(output_path) as writer:
    mean_sd_table.to_excel(writer, sheet_name="descriptive", index=False)
